...
<form id='form-table' method='post' action='{{CRUDBooster::mainpath("action-selected")}}'>
    ...
    <table id='table_dashboard' class="table table-hover table-striped table-bordered custom-table">
        <thead>
        <tr class="active">
            <?php echo"<th class='v-m' rowspan='2' style=\"width:44px;\">No</th>";?>
            <?php if($button_bulk_action):?>
            <th width='3%' rowspan='2' class='v-m'><input type='checkbox' id='checkall'/></th>
            <?php endif;?>
            <?php if($show_numbering):?>
            <th width="1%" rowspan='2' class='v-m'>{{ trans('crudbooster.no') }}</th>
            <?php endif;?>
            <?php
            foreach ($columns as $col) {
                ...
                if($col['position']==1){
                if($colspan==null)
                    echo "<th width='$width' rowspan='2' class='v-m'>";
                else
                    echo "<th width='$width' colspan='$colspan' class='v-m'>";
            ...
            @if($button_table_action)
                @if(CRUDBooster::isUpdate() || CRUDBooster::isDelete() || CRUDBooster::isRead())
                    <th width='{{$button_action_width?:"auto"}}' rowspan="2" class="v-m" style="text-align:center">{{trans("crudbooster.action_label")}}</th>
                @endif
            @endif
        </tr>
        <tr class="active">
        ...
        <tr>
            ...
            <th class="text-center" colspan="2">6A</th>
        ...
        @if(count($result)==0)
            <tr class='warning'>
                ...
                <td colspan='{{count($columns)+4}}' align="center">
        ...
        <?php $no++; ?>
                <tr>
                    <td class="text-center"><?=((($result->currentPage())-1)*($_GET['limit']==null?10:$_GET['limit']))+$no;?></td>
                    @endif
                    @foreach($hc as $h)
                        @if($h =='')
...
$total = $result->total();
?>
<div class="col-md-4 custom-total-rows"><span>{{ trans("crudbooster.filter_rows_total") }}
        : {{ $from }} {{ trans("crudbooster.filter_rows_to") }} {{ $to }} {{ trans("crudbooster.filter_rows_of") }} {{ $total }}</span></div>
<div class="col-md-8 cutom-pagination">{!! urldecode(str_replace("/?","?",$result->appends(Request::all())->render())) !!}</div>
