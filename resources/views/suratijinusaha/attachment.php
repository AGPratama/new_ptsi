    ...
    <div class="modal-dialog modal-lg">
        <div class="modal-content">
            <div class="modal-header">
                ...
                <h4 class="modal-title" id="myModalLabel">Surat Ijin Usaha</h4>
            </div>
            <div class="modal-body center-obj">
                <?php if ($row->surat_ijin !== null) { ?>
                    <?php if(strpos($row->surat_ijin, 'pdf')!==FALSE){ ?>
                        <object data="<?= url($row->surat_ijin) ?>" type="application/pdf" style="width:100%" height="500">
                            <embed src="<?= url($row->surat_ijin) ?>" style="width:100%" height="500px" />
                            <p>This browser does not support PDFs. Please download the PDF to view it:
                                <a href="<?= url($row->surat_ijin) ?>">Download PDF</a>.</p>
                            </embed>
                        </object>
                    <?php } else { ?>
                        <img class="img-responsive" src="<?= url($row->surat_ijin) ?>">
                    <?php } ?>
                <?php }else{ ?>
