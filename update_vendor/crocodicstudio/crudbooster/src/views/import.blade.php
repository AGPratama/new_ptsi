                        ...
                        $(function () {
                            ...
                            $.post("{{ CRUDBooster::mainpath('do-import-chunk').'?file='.Request::get('file') }}", function (resp) {
                                if (resp.status == true) {
                                    ...
                                    $('#progress-import').attr('aria-valuenow', 100);
                                    setTimeout(function(){
                                    $('#status-import').addClass('text-danger').html("<i class='fa fa-eject'></i> Import Data Failed ! <br>"+resp.message);
                                    },3000);
                                    $('#upload-footer').show();
                    ...
                    <table class='table table-bordered' style="width:130%">
                        ...
                        <tr class='success'>
                            @foreach($table_columns as $k=>$column)
                                <?php
                                $help = '';
                                if ($column == 'id' || $column == 'created_by' || $column == 'updated_by' || $column == 'created_at' || $column == 'updated_at' || $column == 'deleted_at') continue;
                        ...
                        <tr>
                            @foreach($table_columns as $k=>$column)
                                <?php if ($column == 'id' || $column == 'created_by' || $column == 'updated_by' || $column == 'created_at' || $column == 'updated_at' || $column == 'deleted_at') continue;?>
                                <td data-no-column='{{$k}}'>
